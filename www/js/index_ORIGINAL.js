...
function saveNote() {
  ...
  noteFechaHora = fecha.getDate()+"/"+fecha.getMonth()+"/"+fecha.getFullYear()+" "+fecha.getHours()+":"+minutos(fecha.getMinutes());
  ...
  if (noteText != "") {
    ...
    arrayNotas.push({texto: noteText, image: noteImg, fecha: noteFechaHora, latitude: noteLtd, longitude: noteLng});
...
function cargarNotas() {
  ...
  if (mostrarNotas.length > 0) {
    ...
    for (var i=0; i<=mostrarNotas.length-1; i++) { 
      ...
      fh.setAttribute('id', 'fecha');
      fh.textContent = mostrarNotas[i].fecha;
      item.appendChild(fh);
...
function cargarNota(){
  ...
  fh.textContent = mostrarNotas[id].fecha + " en ";

  string.setAttribute('id', 'textoNota');
  string.textContent = mostrarNotas[id].texto;
  laNota.appendChild(string);
